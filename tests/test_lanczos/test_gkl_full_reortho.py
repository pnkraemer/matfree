...
def test_gkl_full_reortho(A, order):
    ...
    alg = lanczos.gkl_full_reortho(order, matrix_shape=np.shape(A))
...
def test_error_too_high_depth(A):
    ...
    with testing.raises(ValueError):
        _ = lanczos.gkl_full_reortho(max_depth + 1, matrix_shape=np.shape(A))
...
def test_error_too_low_depth(A):
    ...
    min_depth = 0
    with testing.raises(ValueError):
        _ = lanczos.gkl_full_reortho(min_depth - 1, matrix_shape=np.shape(A))
...
def test_no_error_zero_depth(A):
    ...
    nrows, ncols = np.shape(A)
    algorithm = lanczos.gkl_full_reortho(0, matrix_shape=np.shape(A))
    key = prng.prng_key(1)
    v0 = prng.normal(key, shape=(ncols,))
