...
from hutch.backend import np, prng, testing, transform
...
def test_trace_and_diagonal(fn, rng_key, num_samples, dim, generate_samples_fn):
    ...
    assert np.allclose(diag, np.diag(J)), np.linalg.norm(diag - np.diag(J))
    assert np.allclose(trace, np.trace(J)), np.linalg.norm(trace - np.trace(J))


#
# @testing.parametrize("num_batches", [1, 1_000])
# @testing.parametrize("batch_size", [1, 1_000])
# @testing.parametrize("dim", [1, 100])
# @testing.parametrize(
#     "generate_samples_fn", [prng.normal, prng.rademacher]
# )
...
#     )
#     truth = np.trace(J)
#     assert np.allclose(estimate, truth, atol=1e-1, rtol=1e-1)
